...
final class Bulk_Sync implements Registrable {
	...
	public function register_hooks(): void {
		add_action( 'etherlabz_intercom_bulk_sync_cron', array( $this, 'run_batch' ) );
		add_action( 'etherlabz_intercom_bulk_sync_batch', array( $this, 'run_batch' ) );
	}
	...
	public function run_batch(): void {
		...
		if ( empty( $customers ) ) {
			...
			delete_option( 'etherlabz_intercom_bulk_sync_offset' );
			update_option( 'etherlabz_intercom_bulk_sync_running', 'no' );
		...
		update_option( 'etherlabz_intercom_bulk_sync_offset', $new_offset );
		...
		if ( ! wp_next_scheduled( 'etherlabz_intercom_bulk_sync_batch' ) ) {
			wp_schedule_single_event( time() + 5, 'etherlabz_intercom_bulk_sync_batch' );
		}
	}
	...
	public static function start(): void {
		update_option( 'etherlabz_intercom_bulk_sync_offset', 0 );
		update_option( 'etherlabz_intercom_bulk_sync_running', 'yes' );
		Intercom_API::log( 'success', 'bulk-sync', 'Bulk sync started.' );

		// Fire the first batch immediately via a single cron event.
		if ( ! wp_next_scheduled( 'etherlabz_intercom_bulk_sync_batch' ) ) {
			wp_schedule_single_event( time(), 'etherlabz_intercom_bulk_sync_batch' );
		}
	...
	public static function is_running(): bool {
		return 'yes' === get_option( 'etherlabz_intercom_bulk_sync_running', 'no' );
	}
}
